...
| `MARINARA_ENV_WATCH`             | `1`                                            | How the server watches `.env` for live changes. Default: instant change events. `poll` checks the file every 30 seconds instead (for network mounts or bind mounts edited from outside). `0` turns watching off; changes then need a restart. See [Restart or hot reload](#restart-or-hot-reload).                                                                                                                                                                                |
