...
- Added host-owned `GET`/`PUT /api/game/:chatId/experience-state` routes so a game-surface Experience package can store its world state in the engine's per-message `game_engine_state` table instead of chat metadata: saves are scoped to the chat's stamped Experience (`experience:<id>` rows, never visible to or from turn-games), anchored to the visible message so swipes, edits, branches, and checkpoint restores rewind the world exactly like a turn-game, and bounded per save (#5102).
