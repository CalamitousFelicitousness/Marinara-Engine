      ...
      result.mode === "dry-run"
        ...
        : saved
          ? result.approval?.status === "pending"
            ? "Applied and saved. Marinara is showing the user a Keep/Restore review card. Verify the resulting state with a read command before claiming user-visible success - matter-of-factly, never as an apology or correction. Next time stage the confirmatory read in the same response as the write; commands run in order, so that verifies with no extra round."
            : "Applied and saved. Verify the resulting state with a read command before claiming user-visible success - matter-of-factly, never as an apology or correction. Next time stage the confirmatory read in the same response as the write; commands run in order, so that verifies with no extra round."
          : undefined,
