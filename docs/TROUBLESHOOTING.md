...
- **Could not reach the speech engine** means the address refused the connection. Check that the engine is running and the Base URL is right. Loopback and LAN addresses both work as they are, unless the server sets `TTS_LOCAL_URLS_ENABLED=false`.
