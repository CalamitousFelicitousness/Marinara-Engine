...
The entry saves on its own a moment after you stop typing. You will see a short **Saved automatically** note. Your lorebook now works: when a keyword matches the recent chat, the entry's content joins the prompt. The entries guide explains keywords, matching rules, and timing options, and its **Authoring strategy** and **Worked example** sections show how to choose the right controls for each entry.
