...
- Game Mode rerolls now restart narration at the beginning of the newly saved swipe, generated Timeline and Library journal entries can be edited, and the scene analyzer can repeat a beat's sound effect from one to five total plays. Earlier branches retain the inventory/widget rewind behavior already shipped on `staging` (#5287, #5327).
