...
- Professor Mari's plans and questions no longer vanish into hidden reasoning on custom connections (#5721): the hidden-reasoning disable that protects her JSON command protocol now covers custom providers too, so a reasoning-capable model on a local OpenAI-compatible server (e.g. Unsloth-served GGUF builds) answers in the visible reply instead of burying its brainstorming in the reasoning channel. Endpoints that reject reasoning parameters can opt out by disabling the reasoning-effort parameter on the connection.
