...
- Added capability API 1.12: spatial transition capability events (`spatial_transition_committed`, `spatial_transition_rejected`, and the `spatial_context_refresh` nudge) are now also addressed to the game-owning Experience package, so an Experience that sent a travel command learns the outcome the moment the host does instead of inferring it from later state reads. Transitions rejected on the pre-stream commit path — previously a plain HTTP error with no event at all, a gap that affected World Maps too — now synthesize a `spatial_transition_rejected` event carrying the error code (#5143).
