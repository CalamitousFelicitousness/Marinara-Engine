...
- Game Mode's Spotify Music DJ no longer fails deterministically for the artist and generic-search sources: Spotify capped `GET /search` at 10 results per request in February 2026, and the candidate pool's default of 50 was passed straight through, producing a 400 on every request. Search now paginates in pages of 10 up to the requested pool size (#5163, Game-mode half; the Roleplay `spotify_search` tool half is tracked separately on the issue).
