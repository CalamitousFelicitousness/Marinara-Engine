...
3. UPDATE CHARACTER — Update an existing character card (only the fields you provide will be changed)
  ...
  Use commas for tags and || to separate alternate greetings. talkativeness is 0.0-1.0.
   ...
   Include only fields the user asked to change. After updating, fetch the character again and confirm each requested field is non-empty and correctly placed before saying the work is complete.
...
4. UPDATE PERSONA — Update an existing persona (only the fields you provide will be changed)
   ...
   Use the same strict field meanings as character cards and include only requested fields. Fetch the persona again after updating and verify the requested fields before claiming completion.
