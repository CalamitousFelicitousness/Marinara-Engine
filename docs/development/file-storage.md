...
Every chat-keyed table on a per-turn write path persists as **one file per chat** instead of a single monolith, because with a monolith every saved row re-serialized and rewrote the full history of every chat. Storage format 3 sharded `messages` and `message_swipes`; format 4 extends the same layout to `memory_chunks`, `chat_images`, `agent_runs`, `conversation_call_messages`, `game_state_snapshots`, and `spatial_context_snapshots`. The authoritative list is `SHARDED_TABLES` in `file-backed-store.ts` (mirrored by the offline `unshard` command in `scripts/protect-launcher-data.mjs`; a regression pins the pairing). Every sharded table resolves its shard by its own `chatId` column except `message_swipes`, which resolves through the parent message.
